...
bool SensitiveOperations::runOnModule(Module &M) {
  ...
  for (Module::iterator function = M.begin(), FEnd = M.end(); function != FEnd;
       function++) {
  ...
  //找pthread_create的参数，找pthread_mutex_lock, atomic_fetch_add,
  // atomic_fetch_sub, atomic_exchange
  ...
  for (Module::iterator function = M.begin(), FEnd = M.end(); function != FEnd;
    ...
    for (Function::iterator basicblock = function->begin(),
      ...
      for (BasicBlock::iterator instruction = basicblock->begin(),
                                IEnd = basicblock->end();
           instruction != IEnd; instruction++) { //遍历每一条instruction
        ...
        if (Instruction *inst = dyn_cast<Instruction>(instruction)) {
          ...
          if (inst->getOpcode() == Instruction::Call) {
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于2
              if (str_free ==
                  std::string(inst->getOperand(1)->getName())) { // free
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于2
              if (str_mutex_lock ==
                  std::string(
                      inst->getOperand(1)->getName())) { // pthread_mutex_lock
                std::string SourceLoc = getSourceLoc(inst);
                if (SourceLoc.size() ==
                    0) // SourceLoc为空则不用比较了，直接continue
                  continue;
                ...
                for (int i = 0; i < LockFunctionNum; i++) {
                  if (LockFunctionString[i] == function->getName() ||
                      LockFunctionString[i] == strfnameDemangled) {
                ...
                if (isLockFunction == 0) {
                  if (function->size() == 1 && function->arg_size() == 1 &&
                      function->getInstructionCount() <= 10 &&
                      function->isVarArg() == false &&
                      function->hasAddressTaken() ==
                          false) { // Heuristic judgment
                    LockFunctionSet.insert(&*function);
                    isLockFunction = 1;
                  }
                }
              ...
              } else if (str_mutex_unlock ==
                         std::string(inst->getOperand(1)
                ...
                for (int i = 0; i < LockFunctionNum; i++) {
                  if (UnlockFunctionString[i] == function->getName() ||
                      UnlockFunctionString[i] == strfnameDemangled) {
                ...
                for (int i = 0; i < LockFunctionNum; i++) {
                  if (LockFunctionString[i] ==
                          std::string(inst->getOperand(1)->getName()) ||
                      LockFunctionString[i] == strfnameDemangled) {
                    Function *function_of_lock = dyn_cast<Function>(
                        inst->getOperand(1)->stripPointerCasts());
                    LockFunctionSet.insert(&*function_of_lock);
                    pthreadFunctionMap[&*function] =
                        true; // Open a back door for interested lock functions,
                              // regardless of thread creation and function
                              // calls
                    break;
                  }
                }
            ...
            std::string str_atomic_fetch_add =
                "long std::atomic_fetch_add<long>";
            std::string str_atomic_fetch_sub =
                "long std::atomic_fetch_sub<long>";
            std::string str_atomic_exchange1 = "std::atomic<void*>::exchange";
            std::string str_atomic_exchange2 =
                "webrtc::SSRCDatabase* "
                "std::atomic_exchange<webrtc::SSRCDatabase*>";
            if (inst->getNumOperands() == 3 ||
                inst->getNumOperands() ==
                    4) { //操作数大于3, 有类对象是操作数为4，因为还有this
            ...
            if (inst->getNumOperands() == 5) { //操作数等于5
              if (boost::algorithm::starts_with(
                      std::string(inst->getOperand(4)->getName()),
                      memsetstring)) {
                std::string SourceLoc = getSourceLoc(inst);
                if (SourceLoc.size() ==
                    0) // SourceLoc为空则不用比较了，直接continue
          ...
          /*else if (inst->getOpcode() == Instruction::ICmp) {
              ...
              if(inst->getNumOperands() == 2){ //操作数等于5
                  if (ConstantPointerNull *CI =
          dyn_cast<ConstantPointerNull>(inst->getOperand(1))){ std::string
          SourceLoc = getSourceLoc(inst); if (SourceLoc.size() == 0)
          //SourceLoc为空则不用比较了，直接continue continue; if
          (SourceLoc.find(":")== std::string::npos) //SourceLoc可能取得有问题
                        continue;
                      std::string tmpstring("./");
                      if (SourceLoc.compare(0, tmpstring.length(), tmpstring) ==
          0) {//如果SourceLoc前两个字符为"./", 则删除掉 SourceLoc.erase(0,2);
                      }
...
#ifdef LLVM_11
              string strfname =
                  inst->getOperand(inst->getNumOperands() - 1)->getName().str();
#else
              string strfname =
                  inst->getOperand(inst->getNumOperands() - 1)->getName();
#endif
              string strfnameDemangled =
                  get_funcname(strfname.c_str()); // C++函数名解码
              size_t iIndex = strfnameDemangled.rfind("(");
              if (iIndex != std::string::npos)
                strfnameDemangled = strfnameDemangled.substr(0, iIndex);
              if (boost::algorithm::istarts_with(strfnameDemangled,
                                                 VecResizeStart) &&
                  boost::algorithm::ends_with(strfnameDemangled,
                                              VecResizeEnd)) {
                std::string SourceLoc = getSourceLoc(inst);
                if (SourceLoc.size() ==
                    0) // SourceLoc为空则不用比较了，直接continue
        ...
        if (Instruction *inst = dyn_cast<Instruction>(instruction)) {
          ...
          if (inst->getOpcode() == Instruction::Call) {
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              std::set<llvm::Function *>::iterator
                  LockFunctionSetiter; //定义前向集合迭代器
              for (LockFunctionSetiter = LockFunctionSet.begin();
                   LockFunctionSetiter != LockFunctionSet.end();
                   LockFunctionSetiter++) {
#ifdef LLVM_11
                std::string LockFunctionstring =
                    (*LockFunctionSetiter)->getName().str();
#else
                std::string LockFunctionstring =
                    (*LockFunctionSetiter)->getName();
#endif

                if (LockFunctionstring ==
                    std::string(inst->getOperand(1)->getName())) {
                  std::string SourceLoc = getSourceLoc(inst);
                  if (SourceLoc.size() ==
                      0) // SourceLoc为空则不用比较了，直接continue
  ...
  CallGraph &CG =
      Pass::getAnalysis<CallGraphWrapperPass>().getCallGraph(); //获取Call graph
  ...
  while (isFixPoint == 0) { //不动点算法
    isFixPoint = 1;
    std::set<llvm::Function *>::iterator
        functionInterestingiter; //定义前向集合迭代器
    for (functionInterestingiter = pthreadAllCalleeSet.begin();
         functionInterestingiter != pthreadAllCalleeSet.end();
         functionInterestingiter++) {
      if (SearchCalleeFinishSet.find(*functionInterestingiter) ==
          SearchCalleeFinishSet.end()) { //没有被找过Callee
        SearchCalleeFinishSet.insert(
            *functionInterestingiter); //加入集合保存，用于不动点迭代不重复搜索，后面遇到这个函数就不用再找了
        CallGraphNode *CallerNode =
            CG.getOrInsertFunction(*functionInterestingiter);
#ifdef DEBUG_INFO_OUT
        errs() << "[DEBUG] Calculating "
               << (*functionInterestingiter)->getName() << "'s Callee. "
               << "  #uses=" << CallerNode->getNumReferences() << '\n';
#endif
        ...
        for (const auto &I : *CallerNode) {
          if (Function *FI = I.second->getFunction()) {
            if (FI->size() != 0) {
...
#ifdef DEBUG_INFO_OUT
                errs() << "[DEBUG]   " << callinst->getParent()->getName()
                       << " calls function '" << FI->getName() << "'\n";
#endif
                outs() << "[DEBUGDEBUG]   " << callinst->getParent()->getName()
                       << " calls function '" << FI->getName() << "'\n";
              ...
              // errs() << "  CS<" << I.first << "> calls " << "external
              // node\n";
              continue;
            }
          }
        ...
        for (Function::iterator
                 basicblock = (*functionInterestingiter)->begin(),
                 BBEnd = (*functionInterestingiter)->end();
             basicblock != BBEnd; basicblock++) { //遍历每一个Basic Block
          for (BasicBlock::iterator instruction = basicblock->begin(),
                                    IEnd = basicblock->end();
               instruction != IEnd; instruction++) { //遍历每一条instruction
            if (Instruction *inst = dyn_cast<Instruction>(instruction)) {
              if (inst->getOpcode() == Instruction::Call) {
                if (CallInst *callfunInst = dyn_cast<CallInst>(instruction)) {
                  Function *callfun = callfunInst->getCalledFunction();
                  if (callfun) {
                    ; // the same as else branch, but is done above (see Seeking
                      // Callee)
                  } else {
                    callfun = dyn_cast<Function>(
  ...
  if (ConcurrencyInsteresting == 1) {
    ...
    for (int i = 0; getline(read_file, line); i++) {
      ...
      lock_index = 0;
      free_index = 0;
      for (Coniter = mutexLockSet.begin();
           Coniter != mutexLockSet.end() && lock_index < lockNum;
           lock_index++) {
      ...
      for (Coniter = SoFuncSet.begin();
           Coniter != SoFuncSet.end() && free_index < SoNum; free_index++) {
        if (ConFirst[i] == *Coniter) {
    ...
    for (Coniter = SoFuncSet.begin();
         Coniter != SoFuncSet.end() && free_index < SoNum;
         Coniter++, free_index++) {
    ...
    for (Module::iterator function = M.begin(), FEnd = M.end();
         function != FEnd; function++) { //遍历每一个Function
      ...
      if (canSeekCallee_global_flag == false) {
        ...
        if (function->isDeclaration() || function->size() == 0 ||
            pthreadFunctionMap[&*function] == false) {
          // outs() << "[DEBUG] " << function->getName() << " is filtered" <<
          // "\n";
          continue;
        }
      ...
      outs() << "Function: " << function->getName()
             << " " + getSourceLoc(&*(function->begin()->begin())) << "\n";
      for (Function::iterator basicblock = function->begin(),
                              BBEnd = function->end();
           basicblock != BBEnd; basicblock++) { //遍历每一个Basic Block
            ...
            for (int i = 0; i < linesNum + lockNum + SoNum; i++) {
              ...
              if (boost::algorithm::ends_with(
...
#ifdef DEBUG_INFO_OUT
                outs() << "[DEBUG] Match to diff in function "
                       << function->getName() << ":" << SourceLoc << "\n";
                outs() << "[DEBUG]   " << *inst << " (in BasicBlock "
                       << basicblock->getName() << ")"
                       << "\n";
#endif
                ...
                if (inst->getOpcode() == Instruction::Call) {
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (instr_malloc1 ==
                        ...
                        instr_malloc6 ==
                            std::string(inst->getOperand(1)->getName())) {
                      outs() << ConLine[i] << " malloc " << function->getName()
                             << " " << splitConLine << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 3 && !FindFlag) { //操作数大于二
                    if (instr_calloc1 ==
                            std::string(inst->getOperand(2)->getName()) ||
                        instr_calloc2 ==
                            std::string(inst->getOperand(2)->getName()) ||
                        instr_calloc3 ==
                            std::string(inst->getOperand(2)->getName()) ||
                        ...
                        instr_calloc7 ==
                            std::string(inst->getOperand(2)->getName())) {
                      outs()
                          << ConLine[i] << " malloc " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 3 && !FindFlag) { //操作数大于二
                    if (instr_realloc1 ==
                        std::string(inst->getOperand(2)->getName())) {
                      outs()
                          << ConLine[i] << " realloc " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (instr_free1 ==
                        ...
                        instr_free7 ==
                            std::string(inst->getOperand(1)->getName())) {
                      outs()
                          << ConLine[i] << " free " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (instr_new1 ==
                        ...
                        instr_new4 ==
                            std::string(inst->getOperand(1)->getName())) {
                      outs()
                          << ConLine[i] << " new " << function->getName() << " "
                          << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (instr_delete1 ==
                        ...
                        instr_delete2 ==
                            std::string(inst->getOperand(1)->getName())) {
                      outs()
                          << ConLine[i] << " delete " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() == 5 && !FindFlag) { //操作数等于5
                    if (boost::algorithm::starts_with(
                            ...
                            memsetstring)) {
                      outs()
                          << ConLine[i] << " memset " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() == 5 && !FindFlag) { //操作数等于5
                    if (boost::algorithm::starts_with(
                            ...
                            memcpystring)) {
                      outs()
                          << ConLine[i] << " memcpy " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (mutexLockstring ==
                      ...
                      outs()
                          << ConLine[i] << " lock " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    ...
                    for (LockFunctionSetiter = LockFunctionSet.begin();
                      ...
                      if (LockFunctionstring ==
                          std::string(inst->getOperand(1)->getName())) {
                        outs() << ConLine[i] << " lock " << function->getName()
                               << " " << string_hash_encoding(splitConLine)
                               << "\n";
                        FindFlag = 1;
                      }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (mutexUnlockstring ==
                        std::string(inst->getOperand(1)->getName())) {
                      // outs() << *inst << "\n";
                      // outs() << *inst->getOperand(0) << "\n";
                      outs()
                          << ConLine[i] << " unlock " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                  ...
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    std::set<llvm::Function *>::iterator
                        UnlockFunctionSetiter; //定义前向集合迭代器
                    for (UnlockFunctionSetiter = UnlockFunctionSet.begin();
                         UnlockFunctionSetiter != UnlockFunctionSet.end();
                         UnlockFunctionSetiter++) {
#ifdef LLVM_11
                      std::string UnlockFunctionstring =
                          (*UnlockFunctionSetiter)->getName().str();
#else
                      std::string UnlockFunctionstring =
                          (*UnlockFunctionSetiter)->getName();
#endif
                      if (UnlockFunctionstring ==
                          std::string(inst->getOperand(1)->getName())) {
                        outs() << ConLine[i] << " unlock "
                               << function->getName() << " "
                               << string_hash_encoding(splitConLine) << "\n";
                        FindFlag = 1;
                      }
                  ...
                  std::string str_atomic_fetch_sub =
                      "long std::atomic_fetch_sub<long>";
                  std::string str_atomic_exchange1 =
                      "std::atomic<void*>::exchange";
                  std::string str_atomic_exchange2 =
                      "webrtc::SSRCDatabase* "
                      "std::atomic_exchange<webrtc::SSRCDatabase*>";
                  if ((inst->getNumOperands() == 3 ||
                    ...
                    if (CallInst *callfunInst =
                      ...
                      if (boost::algorithm::starts_with(strfnameDemangled,
                                                        str_atomic_fetch_add) ||
                          boost::algorithm::starts_with(strfnameDemangled,
                                                        str_atomic_fetch_sub) ||
                          boost::algorithm::starts_with(strfnameDemangled,
                                                        str_atomic_exchange1) ||
                          boost::algorithm::starts_with(strfnameDemangled,
                                                        str_atomic_exchange2)) {
                        outs() << ConLine[i] << " std_atomic "
                               << function->getName() << " "
                               << string_hash_encoding(splitConLine) << "\n";
                        FindFlag = 1;
                      }
                  ...
                  // std::Map.size函数
                  std::string str_std_Map_size =
                      "std::map<unsigned int, unsigned int, std::less<unsigned "
                ...
                if (inst->getOpcode() == Instruction::Invoke) {
                  ...
                  if (inst->getNumOperands() == 5 &&
                    ...
                    if (boost::algorithm::istarts_with(strfnameDemangled,
                        ...
                        boost::algorithm::ends_with(strfnameDemangled,
                                                    VecResizeEnd)) {
                      outs() << ConLine[i] << " std::vector<...>::resize "
                             << function->getName() << " "
                             << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                ...
                if (inst->getOpcode() == Instruction::Store) {
                  if (inst->getNumOperands() >= 2 && !FindFlag) { //操作数大于二
                    if (ConstantPointerNull *CI = dyn_cast<ConstantPointerNull>(
                            inst->getOperand(0))) {
                      outs()
                          << ConLine[i] << " nullptr " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                ...
                if (inst->getOpcode() == Instruction::ICmp) {
                  if (inst->getNumOperands() == 2 && !FindFlag) { //操作数等于5
                    if (ConstantPointerNull *CI = dyn_cast<ConstantPointerNull>(
                            inst->getOperand(1))) {
                      outs()
                          << ConLine[i] << " cmpnull " << function->getName()
                          << " " << string_hash_encoding(splitConLine) << "\n";
                      FindFlag = 1;
                    }
                  }
                ...
                if (!FindFlag) {
                  outs() << ConLine[i] << " R/W " << function->getName() << " "
                         << string_hash_encoding(splitConLine) << "\n";
                }
