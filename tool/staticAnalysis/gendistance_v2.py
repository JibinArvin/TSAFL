...
def parse_graph(
    ...
    for node_id in list(graph.nodes):
        ...
        # print("Inside lable--------------")
        # print("function")
        # print(lable_temp.function)
        # print("loc")
        # print(lable_temp.loc)
        node_temp = config.Node(
            node_id, lable_temp.loc, lable_temp.function, lable_temp
        )
        node_temp.change_important()
        node_dict[node_id] = deepcopy(node_temp)
        mygraph.add_node(deepcopy(node_temp))
    ...
    if(mygraph.edge_out_map == None):
        return
    mygraph.generate_path(graph, function_set, operation_dict, kk_mem)
    # mygraph.add_node()
...
def main():
    ...
    config.kk_mem_to_xml(
        "distance.xml", operation_dict, node_dict, functions_dict, kk_mem
    )
