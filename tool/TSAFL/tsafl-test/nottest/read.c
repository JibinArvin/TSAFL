  ...
  if (cur != NULL) {
    ...
    while (cur != NULL) {
      if (!xmlStrcmp(cur->name, (xmlChar *)"End")) {
        ...
        dis_node->path[dis_node->size] = (struct distance_node_info*)ck_alloc(sizeof(struct distance_node_info));
...
int parse_distance_xml(const char *file_name, struct distance_array *d_array) {
  ...
  while (cur != NULL) {
    if (!xmlStrcmp(cur->name, (const xmlChar *)"Start")) {
      ...
      struct distance_container *d_c_temp = (struct distance_container *)ck_alloc(
          sizeof(struct distance_container));
      d_c_temp->size = 0;
      memset(d_c_temp->nexter, -1, NODE_REALATION_MAX_NUMBER*sizeof(int));
      // TODO: change
      memset(d_c_temp->path, 0, NODE_REALATION_MAX_NUMBER*sizeof(struct distance_node_info*));
...
int parse_root_node_sensitiveXML(const char *file_name,
  ...
  while (cur != NULL) {
    ...
    if (!xmlStrcmp(cur->name, (const xmlChar *)"Value")) {
      ...
      struct variable_container *v_c_tem = (struct variable_container *)ck_alloc(
...
int parse_groupKey_xml(const char *file_name, struct groupKey_array *g_array) {
  ...
  while(cur != NULL) {
    if(!xmlStrcmp(cur->name, (const xmlChar *)"KEY")) {
      ...
      struct group_key_container* g_item = (struct group_key_container* )ck_alloc(sizeof(struct group_key_container));
...
static void dump_g_array(struct groupKey_array *g_array) {
    ...
    for(int i =0; i<OP_MAX_COUNT;i++) {
        ...
        printf("g_array[%d]->key: %d, size:%d.\n", i ,g_array->list[i]->key, g_array->list[i]->size);
    }
}
