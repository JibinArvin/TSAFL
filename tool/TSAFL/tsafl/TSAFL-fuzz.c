...
EXP_ST u8 skip_deterministic, /* Skip deterministic stages?       */
    ...
    fast_cal;                 /* Try to calibrate faster? JIBIN disable this.*/
...
EXP_ST u64 total_crashes, /* Total number of crashes          */
    ...
    start_time_s,         /* Unix start time (s)             */
...
void update_time_cost_target(u64 single_time) {
  if (time_cost_run_target_single == 0) {
    time_cost_run_target_single = single_time;
    return;
  }
  time_cost_run_target_single = (time_cost_run_target_single + single_time) / 2;
}

/* Get unix time in seconds */
static u64 get_cur_time_s(void) {
...
/* JIBIN: The queue_entry is inited by zero!*/
/* Append new test case to the queue. JIBIN: no change. But in final running
 * state per_100 is disabled. Cause we add multi-target. */
...
static void update_bitmap_score(struct queue_entry *q) {
  ...
  u64 fuzz_p2 = next_p2(q->exec_time);
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
      ...
      if (top_rated[i]) {
        ...
        u64 top_rated_fuzz_p2 = next_p2(top_rated[i]->exec_time);
...
/* JIBIN: Here do the energy allocation for every seed file.
 */
...
static void cull_queue(void) {
  ...
  for (i = 0; i < MAP_SIZE; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      top_rated[i]->tr_intersting = 1;

      if (top_rated[i]->fuzz_level == 0)
        pending_favored++;
    }

  q = queue;

  set_queueEntry_interesting_WandC();

  while (q) {
    if (q->tr_intersting || q->sch_interesting) {
      q->favored = 1;
      queued_favored++;
    }
    q = q->next;
  ...
  /* Out message. */
  TSF("Finish cull once time.");
  TSF("queued_favored size: %d", queued_favored);
}
...
void clean_xml_info() { clean_xml_config(f_array, v_array, d_array, g_array); }
/* add finished */
...
static void read_testcases(void) {
  ...
  /* Shuffle_queue is useless in TSAFL. JIBIN */
...
EXP_ST void init_forkserver(char **argv) {
  ...
  if (WIFSIGNALED(status)) {
    ...
    TSF("chile process receive signal %d\n", WTERMSIG(status));
  ...
  if (*(u32 *)trace_bits == EXEC_FAIL_SIG)
    FATAL("The trace_bits is equal to EXEC_FAIL_SIG. Unable to execute target "
          "application ('%s')",
          argv[0]);
...
/* JIBIN: New part: 1. add signal to make sure there if need to run case under
 * guide!*/
static u8 run_target(char **argv, u32 timeout) {
  ...
  /* After this memset, trace_bits[] are effectively volatile, so we
     ...
     territory. */
  /* JIBIN: clear shared memory. NOT just the memory for trace_bits. */
  ...
  /* JIBIN: Cause TSAFL just run under having forkserver, so no need to change
   * no_forkserver mode.*/

  if (dumb_mode == 1 || no_forkserver) {
    /* JIBIN add. */
    FATAL("JIBIN: Sry we just run under the forkserver!\n");
    /* JIBIN finish. */
    child_pid = fork();
    ...
    if (!child_pid) {
      ...
      /* JIBIN: Cause of the execv the blow will not run in normal situation. */
   ...
   */
  /* JIBIN: Depend on if there is setting the real timer for scheduel. */
  if (need_scheduel == 1) {
    timeout = 1; // TODO:
  }
...
/* Don't need worry about trace_bits here. */
#ifdef __x86_64__
...
/* Insert single result/plan into */
void insert_scheduel_plan(struct scheduel_result *plan,
                          struct Thread_info *t_info) {
  t_info->is_scheduel = 1;
  for (size_t i = 0; i < CONSIDER_THREAD_NUMBER_HELPER; i++) {
    if (plan->result_seq[i] == -1)
  ...
  for (size_t i = 0; i < MY_PTHREAD_CREATE_MAX; i++) {
    ...
    t_info->thread_same_time[i] = plan->thread_same_period[i];
  }
}

void init_shm_sch_part(struct Thread_info *t_info_tmep, u64 dry_run_time) {
  assert(t_info_tmep != NULL);
  if (dry_run_time > 10000000)
    dry_run_time = 10000000;
  t_info_tmep->time = dry_run_time;
  t_info_tmep->thread_care[0] = -1;
  t_info_tmep->thread_care[0] = -1;
  t_info_tmep->thread_same_period_size = 0;
  // memset(t_info_tmep->thread_same_time, -1,
  //        sizeof(int32_t) * MY_PTHREAD_CREATE_MAX);
  memset(t_info_tmep->thread_same_time, -1,
         MY_PTHREAD_CREATE_MAX * sizeof(int32_t));
  memset(t_info_tmep->thread_care, -1, THREAD_NUMBERT_SCH * sizeof(int32_t));
  memset(t_info_tmep->kp_times, 0,
         sizeof(u16) * THREAD_NUMBERT_SCH * SHARE_MEMORY_ARRAY_SIZE);
  memset(t_info_tmep->kp_action, 0,
         sizeof(u16) * THREAD_NUMBERT_SCH * SHARE_MEMORY_ARRAY_SIZE * 4);
  t_info_tmep->kp_mem_size = 0;
}
...
static void write_to_testcase(void *mem, u32 len) {
  ...
  if (!out_file) {
     ...
     * increase the size of the file. If the file size is increased, the
...
static u8 calibrate_case(char **argv, struct queue_entry *q, u8 *use_mem,
  ...
  u8 fault = 0, new_bits = 0, var_detected = 0,
     first_run = (q->exec_cksum == 0), sch_fault = 0;
  ...
  stage_max = 3; /* JIBIN: NO need to repeat that much! */
  TSF("Calibration started, right now!");
  ...
  /* JIBIN: Just after setup struct que, the exec_cksum is 0 or not? */
  if (q->exec_cksum)
    memcpy(first_trace, trace_bits, MAP_SIZE);

  start_us = get_cur_time_us();
  u8 new_window = 0;
  u64 times_start_no_sche, times_end_no_sch;
  times_start_no_sche = get_cur_time();
  ...
  /* update whole fuzzing proccess run target time cost! */
  times_end_no_sch = get_cur_time();
  u64 time_run_single = (times_end_no_sch - times_start_no_sche) / stage_max;
  update_time_cost_target(time_run_single);

  stop_us = get_cur_time_us();
  total_cal_us += stop_us - start_us;
  total_cal_cycles += stage_max;
  u64 ns_one_turn = (stop_us - start_us) / stage_max;

  /* If que don't have new currency performace!*/
  if (new_window == 0) {
  ...
  if (new_window == 1) {
    ...
    u32 use_special_timeout =
        use_tmout * 10000000 < UINT32_MAX
            ? use_tmout * 4
            : UINT32_MAX; /* Expend the time to 4 times more! */
    TSF("Now run run_target_with_scheduel.");

    sch_fault = run_target_with_scheduel(argv, use_special_timeout, use_mem, q,
                                         ns_one_turn);
    if (schedule_var) {
      schedule_var = 0;
      q->schedule_var_be = 1;
...
abort_calibration:
  ...
  /* Mark variable paths. JIBIN : change! */
  ...
  if (new_window == 1) {
    TSF("Find new case have CONCURRENT performance!");
  }
...
static u8 run_target_with_scheduel(char **argv, u32 timeout, u8 *use_mem,
  ...
  u8 fault = 0, round = 0, final_fault = 0;

  stage_name = "Run under command";
  /* Get the info need for runnig under command. */
...
#ifdef OUT_DEBUG_RUNTARGET
#endif
#ifdef LOCK_INDENTIFICATION
/* Since the information about lock we get is exactly what we need, which didn't
 * point out the name of var it protect. So we need test it and get this part
 * info!*/
#endif
  TSF("Before generate_que");
  generate_que(&result, token, tnc);
  /* Check result if OK >_< */
  assert(result != NULL && result->size != 0);
  /* notify the tested program it's time for schedule! */
  enable_schedule();
  if (dumb_mode != 1 && !no_forkserver && !forksrv_pid)
    init_forkserver(argv);

  /*pre-schedule hashcode of the sensitive action by thread for check !*/
  store_info_preSchedule(t_info);
  for (size_t i = 0; i < result->size; i++) {
    TSF("Run target with schedule one more!");
    init_shm_sch_part(t_info, dry_run_time);
    MEM_BARRIER();
    insert_scheduel_plan(result->list[i], info);
    // if (!if_good_plan_now(i))
    //   continue;
    write_to_testcase(use_mem, q->len);
    fault = run_target(argv, timeout);
    if (stop_soon || fault != crash_mode)
      goto abort_run;
    if (!dumb_mode && check_t_info(t_info)) {
      fault = FAULT_NOSCHEDUEL;
      goto abort_run;
    }
    if (!check_after_schedule(t_info)) {
      WARNF("The program run in a different way after schedule!");
      schedule_var = 1;
    }
    int8_t plan_result = check_plan_result(info, result->list[i], i);
    /* extra the windows infomation and add into (helper.cc)g_single_info
     * waiting next step.*/
    size_t plan_size =
  ...
  /* The schedule_var have not being concerned. */
abort_run:
  free(s_i_t);
  free(token);
  free(tnc);
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    TSF("Find new case have TRACE performace!");
#ifndef SIMPLE_FILES
    ...
    queue_top->exec_cksum = exec_cksum;
...
// TOREAD:
static void show_stats(void) { // NEXT:
...
/* calculate_score but in fast way!*/
static u32 calculate_score_fast(struct queue_entry *q) {
  u32 perf_score = calculate_score(q);
  u64 fuzz_time = q->exec_time;
  u32 factor = 1;
  if (q->fuzz_level < 16) {
    factor = ((u32)(1 << q->fuzz_level)) / (fuzz_time == 0 ? 1 : fuzz_time);
  } else {
    factor = MAX_FACTOR / (fuzz_time == 0 ? 1 : next_p2(fuzz_time));
  }
...
static u32 calculate_score_sch(struct queue_entry *q) {
  u32 perf_score = calculate_score(q);
  u64 fuzz_time = q->exec_time;
  u32 factor = 1;
  if (q->fuzz_level < 16) {
    ...
    factor = calculate_score_factor_sch(q);
  }
...
void change_state_enery_byTime() {
  if (state_energy == 1) {
    return;
  }
...
void update_que_size() {
  ...
  que_meaningful_size = final_size;
}

/* Determine whether the current round uses the target splicing by comparison */
int8_t good_time_target_splicing() {
  float rate = (que_meaningful_size - que_meaningful_size_old) * 1.0 /
               que_meaningful_size_old;
...
static u8 fuzz_one(char **argv) {
  ...
  u32 target_splice_stage = 0;
  u32 target_splice_cycle = 0;
  u32 target_limit = 0;
  ...
  switch (state_energy) {
  case 0:
    orig_perf = perf_score = calculate_score_fast(queue_cur);
    break;
  case 1:
    orig_perf = perf_score = calculate_score_sch(queue_cur);
    break;
  default:
    FATAL("[ERROR] unexpected state_energy!");
    break;
  }

  if (perf_score == 0 && queued_paths > 10)
    goto abandon_entry;

  /* Skip right away if -d is given, if we have done deterministic fuzzing on
     this entry ourselves (fuzz_level), or if it has gone through deterministic
...
havoc_stage:
  ...
  if (!splice_cycle && !target_splice_stage) {
  ...
  } else if (target_splice_stage) {

    static u8 tmp[32];
    perf_score = orig_perf;
    sprintf(tmp, "target splice");
    stage_name = tmp;
    stage_short = "target_s";
    stage_max = SPLICE_HAVOC * perf_score / havoc_div / 100;
  }
  ...
  if (target_splice_stage)
    goto target_splicing;
...
/* target_splice is */
target_splicing:
  if (!target_splice_ok && queued_paths <= 1 && queue_cur->len <= 1)
    goto abandon_entry;
  target_splice_stage = 1;
  struct queue_entry **targets;
  u32 targets_size;
  u32 tid, split_at;

  if (target_limit == 0) {
    target_limit = get_target_splicing_limit();
    targets_size = target_limit;
    targets = ck_alloc(targets_size * sizeof(struct queue_entry *));
    int64_t target_re = get_splicing_targets(targets_size, targets);
    if (target_re < targets_size)
      target_limit = target_re;
  }

  if (target_splice_cycle++ < target_limit) {
    u8 *new_buf;
    s32 f_diff, l_diff;
    /*If we've modified in_buf for havoc, let's clean that up...*/
    if (in_buf != orig_in) {
      ck_free(in_buf);
      in_buf = orig_in;
      len = queue_cur->len;
    }
    struct queue_entry *target = targets[target_splice_cycle];

    if (target->len < 2 || target == queue_cur)
      goto target_splicing;
...
static void check_if_tty(void) {
  ...
  FATAL("Sry, guys!, I do nothing for the output~~,"
        " TTY things are not finished!\n");
}
...
void init_helper(struct function_array *f_array, struct variable_array *v_array,
                 struct distance_array *d_array,
                 struct groupKey_array *g_array) {
  TSF("--------------Init_helper here");
  build_g_actionInfo(f_array, v_array, d_array, g_array);
  u32 max_counter = get_max_count_action();
  /* Plus 100 just to avoid acident. */
  build_g_filter(max_counter + 100);
}
...
int main(int argc, char **argv) {
  ...
  readxml();
  init_helper(f_array, v_array, d_array, g_array);
...
#define DRY_RUN
  ...
  while (1) {
    ...
    target_splice_ok = 0;
    if (good_time_target_splicing()) {
      target_splice_ok = 1;
    }

    if (target_splice_ok)
      TSF("This turn is MTM time!");
