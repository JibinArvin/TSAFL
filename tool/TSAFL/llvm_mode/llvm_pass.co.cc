Skip to content Search or
    jump to… Pull requests Issues Codespaces Marketplace Explore

            @aicreatbyme wcventure /
            MemLock -
        Fuzz Public Fork your own copy of wcventure / MemLock -
        Fuzz Code Issues Pull requests Actions Projects Security Insights Beta
            Try the new code view MemLock -
        Fuzz / tool / MemLock / llvm_mode / afl - llvm - pass -
        heap.so.cc @wcventure wcventure MemLock -
        Fuzz Latest commit 6a71879 on Apr 13,
    2020 History 1 contributor Executable File 667 lines(518 sloc) 29.9 KB
...
static inline std::string loc_description(const DebugLoc &dd) {
  ...
  return scope->getFilename().str() + ":" + std::to_string(dd.getLine()) + ":" +
         std::to_string(dd.getCol());
}

static inline std::string bb_description(const BasicBlock &bb) {
  return "(" + loc_description(bb.getInstList().begin()->getDebugLoc()) + "-" +
         loc_description(bb.getTerminator()->getDebugLoc()) + ")";
}
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  if (isatty(2) && !getenv("AFL_QUIET")) {
    ...
    SAYF(cCYA "MemLock-heap-fuzzer: afl-llvm-pass " cBRI VERSION cRST
              " by <wcventure@126.com>\n");
  ...
  llvm::FunctionType *funcFreeType =
      FunctionType::get(builder.getVoidTy(), argTypesRefFree, false);
  llvm::Function *instr_FreeFunc = Function::Create(
      funcFreeType, llvm::Function::ExternalLinkage, "instr_Free", &M);
  ...
  llvm::FunctionType *funcMallocAndSizeType =
      FunctionType::get(builder.getVoidTy(), argTypesRefMalloc, false);
  llvm::Function *instr_MallocAndSizeFunc =
      Function::Create(funcMallocAndSizeType, llvm::Function::ExternalLinkage,
                       "instr_MallocAndSize", &M);
  ...
  llvm::FunctionType *funcCallocAndSizeType =
      FunctionType::get(builder.getVoidTy(), argTypesRefCalloc, false);
  llvm::Function *instr_CallocAndSizeFunc =
      Function::Create(funcCallocAndSizeType, llvm::Function::ExternalLinkage,
                       "instr_CallocAndSize", &M);
  ...
  llvm::FunctionType *funcReallocAhead =
      FunctionType::get(builder.getVoidTy(), argTypesRefReal, false);
  llvm::Function *instr_ReallocAheadFunc =
      Function::Create(funcReallocAhead, llvm::Function::ExternalLinkage,
                       "instr_ReallocAhead", &M);
  ...
  llvm::FunctionType *funcReallocAndSizeType =
      FunctionType::get(builder.getVoidTy(), argTypesRefRealloc, false);
  llvm::Function *instr_ReallocAndSizeFunc =
      Function::Create(funcReallocAndSizeType, llvm::Function::ExternalLinkage,
                       "instr_ReallocAndSize", &M);

  // Function instr_Exit()
  std::vector<Type *> argTypesExit;
  argTypesExit.push_back(builder.getInt32Ty());
  ArrayRef<Type *> argTypesRefExit(argTypesExit);
  llvm::FunctionType *funcExitType =
      FunctionType::get(builder.getVoidTy(), argTypesRefExit, false);
  llvm::Function *instr_ExitFunc = Function::Create(
      funcExitType, llvm::Function::ExternalLinkage, "instr_Exit", &M);
  ...
  GlobalVariable *AFLPrevLocDesc =
      new GlobalVariable(M, CharPtrTy, false, GlobalValue::ExternalLinkage, 0,
                         "__afl_prev_loc_desc", 0,
                         GlobalVariable::GeneralDynamicTLSModel, 0, false);

  ConstantInt *PerfMask = ConstantInt::get(Int32Ty, PERF_SIZE - 1);

  Function *LogLocationsFunc = Function::Create(
      FunctionType::get(VoidTy, ArrayRef<Type *>({CharPtrTy, CharPtrTy}), true),
      GlobalVariable::ExternalLinkage, "__afl_log_loc", &M);
        ...
        if (Instruction *inst = dyn_cast<Instruction>(i)) {
          ...
          if (inst->getOpcode() == Instruction::Call) {
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_malloc1 ==
                  ...
                  instr_malloc6 ==
                      std::string(inst->getOperand(1)->getName())) {
                // outs() << "malloc: Heap memory allocation. " << "(In
                // Function: " << F.getName() << ")\n";
                // MemFuzzBuilder.CreateCall(instr_MallocAndSizeFunc,
                // inst->getOperand(0) ); 此处不插，判断返回值后再插
                insetMallocFlag = 1;
                tmpMallocInst = inst;
                ...
                Value *CurLocDesc =
                    MemFuzzBuilder.CreateGlobalStringPtr(cur_loc_desc);
                ...
                PerfPtr->setMetadata(M.getMDKindID("nosanitize"),
                                     MDNode::get(C, None));
                Value *PerfBranchPtr = MemFuzzBuilder.CreateGEP(
                    PerfPtr, MemFuzzBuilder.CreateAnd(EdgeId, PerfMask));

                /* Increment performance counter for branch */
                LoadInst *PerfBranchCounter =
                    MemFuzzBuilder.CreateLoad(PerfBranchPtr);
                PerfBranchCounter->setMetadata(M.getMDKindID("nosanitize"),
                                               MDNode::get(C, None));
                Value *PerfBranchIncr = MemFuzzBuilder.CreateAdd(
                    PerfBranchCounter, tmpMallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfBranchIncr, PerfBranchPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));

                /* Increment performance counter for total count  */
                LoadInst *PerfTotalCounter = MemFuzzBuilder.CreateLoad(
                    PerfPtr); // Index 0 of the perf map
                PerfTotalCounter->setMetadata(M.getMDKindID("nosanitize"),
                                              MDNode::get(C, None));
                Value *PerfTotalIncr = MemFuzzBuilder.CreateAdd(
                    PerfTotalCounter, tmpMallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfTotalIncr, PerfPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));

                /* Possibly log location */
                LoadInst *PrevLocDesc =
                    MemFuzzBuilder.CreateLoad(AFLPrevLocDesc);
                MemFuzzBuilder.CreateCall(
                    LogLocationsFunc,
                    ArrayRef<Value *>({PrevLocDesc, CurLocDesc}));
            ...
            if (inst->getNumOperands() >= 3) { //操作数大于二
              if (instr_calloc1 ==
                  ...
                  instr_calloc7 ==
                      std::string(inst->getOperand(2)->getName())) {
                // outs() << "calloc: Heap memory allocation. " << "(In
                // Function: " << F.getName() << ")\n"; SmallVector<Value *, 2>
                // CallocArg; CallocArg.push_back(inst->getOperand(0));
                // CallocArg.push_back(inst->getOperand(1));
                // MemFuzzBuilder.CreateCall(instr_CallocAndSizeFunc,
                // CallocArg); 此处不插，判断返回值后再插
                insetCallocFlag = 1;
                tmpCallocInst = inst;
                ...
                PerfBranchCounter->setMetadata(M.getMDKindID("nosanitize"),
                                               MDNode::get(C, None));
                Value *PerfBranchIncr = MemFuzzBuilder.CreateAdd(
                    PerfBranchCounter, tmpCallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfBranchIncr, PerfBranchPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));
                ...
                PerfTotalCounter->setMetadata(M.getMDKindID("nosanitize"),
                                              MDNode::get(C, None));
                Value *PerfTotalIncr = MemFuzzBuilder.CreateAdd(
                    PerfTotalCounter, tmpCallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfTotalIncr, PerfPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));
            ...
            if (inst->getNumOperands() >= 3) { //操作数大于二
              if (instr_realloc1 ==
                  std::string(inst->getOperand(2)->getName())) {
                // outs() << "realloc: Heap memory reallocation. " << "(In
                // Function: " << F.getName() << ")\n";
                ...
                PerfBranchCounter->setMetadata(M.getMDKindID("nosanitize"),
                                               MDNode::get(C, None));
                Value *PerfBranchIncr = MemFuzzBuilder.CreateAdd(
                    PerfBranchCounter, tmpReallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfBranchIncr, PerfBranchPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));
                ...
                PerfTotalCounter->setMetadata(M.getMDKindID("nosanitize"),
                                              MDNode::get(C, None));
                Value *PerfTotalIncr = MemFuzzBuilder.CreateAdd(
                    PerfTotalCounter, tmpReallocSizeInst);
                MemFuzzBuilder.CreateStore(PerfTotalIncr, PerfPtr)
                    ->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_free1 == std::string(inst->getOperand(1)->getName()) ||
                ...
                // outs() << "free: Heap memory release. " << "(In Function: "
                // << F.getName() << ")\n";
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_new1 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new2 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new3 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new4 == std::string(inst->getOperand(1)->getName())) {
                // outs() << "new: Heap memory allocation. " << "(In Function: "
                // << F.getName() << ")\n";
                // MemFuzzBuilder.CreateCall(instr_MallocAndSizeFunc,
                // inst->getOperand(0) );
                insetMallocFlag = 1;
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_delete1 ==
                  ...
                  instr_delete2 ==
                      std::string(inst->getOperand(1)->getName())) {
                // outs() << "delete: Heap memory release. " << "(In Function: "
                // << F.getName() << ")\n";
                MemFuzzBuilder.CreateCall(instr_FreeFunc, inst->getOperand(0));
              }
            }
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_exit == std::string(inst->getOperand(1)->getName())) {
                // outs() << "exit: Program Exit Point. " << "(In Function: " <<
                // F.getName() << ")\n";
          ...
          if (inst->getOpcode() == Instruction::Invoke) {
            ...
            if (inst->getNumOperands() >= 2) { //操作数大于二
              if (instr_malloc1 ==
                      std::string(inst->getOperand(1)->getName()) ||
                  instr_new1 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new2 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new3 == std::string(inst->getOperand(1)->getName()) ||
                  instr_new4 == std::string(inst->getOperand(1)->getName())) {
                // outs() << "new: Heap memory allocation. " << "(In Function: "
                ...
                i++;
                if (i == BB.end()) {
                  BasicBlock *succ_BBptr =
                      BBptr->getTerminator()->getSuccessor(0); //后继
                  BasicBlock::iterator succ_i = succ_BBptr->begin();
                  IRBuilder<> TmpBuilder(
                      &*succ_i); //插桩的位置,invoke跳转的BLock
                  insetMallocFlag = 0;
                  SmallVector<Value *, 2> AllocArg;
                  AllocArg.push_back(tmpMallocInst);
                  AllocArg.push_back(tmpMallocSizeInst);
